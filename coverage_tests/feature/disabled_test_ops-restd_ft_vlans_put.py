...
def setup_test(topology):
    global PATH, vlan_id, vlan_name, vlan_path, vlan
...
@mark.gate
def test_putexistingvlan(step, topology, netop_login, setup_test):
    ops1 = topology.get("ops1")
    assert ops1 is not None
    ...
    ops1("systemctl stop " + daemon_name, shell="bash")
    ops1("cd " + dir, shell="bash")
    ops1("coverage run -a --source=" + dir_name + " " + file_name +  " &", shell="bash")
    sleep(10)
...
@mark.gate
def test_putvlaninvalidname(topology, step, netop_login, setup_test):
...
@mark.gate
def test_putvlaninvalidid(topology, step, netop_login, setup_test):
    data = deepcopy(test_vlan_data)
...
@mark.gate
def test_putvlaninvaliddescription(topology, step, netop_login, setup_test):
    data = deepcopy(test_vlan_data)
...
@mark.gate
def test_putvlaninvalidadmin(topology, step, netop_login, setup_test):
    data = deepcopy(test_vlan_data)
...
@mark.gate
def test_putvlaninvalidotherconfig(topology, step, netop_login, setup_test):
    data = deepcopy(test_vlan_data)
...
def test_putvlaninvalidexternalids(topology, step, netop_login, setup_test):
    data = deepcopy(test_vlan_data)
    ...
    ops1("cd " + dir, shell="bash")
    ops1("ps -ef | grep " + daemon_name + " | grep -v grep | awk '{print $2}' | xargs kill -2", shell="bash")
    ops1("cp .coverage " + dir_name + "/coverage_report/.coverage", shell="bash")
    ops1("systemctl start " + daemon_name, shell="bash")
    sleep(10)
