...
def test_put_vlan_with_star_etag(topology, step, netop_login, setup_test):
    ops1 = topology.get("ops1")
    assert ops1 is not None
    ...
    ops1("coverage run -a --source=" + src_dir_name + " " + file_name +  " &", shell="bash")
    sleep(10)
...
def test_delete_vlan_etag_not_match(topology, step, netop_login, setup_test):
