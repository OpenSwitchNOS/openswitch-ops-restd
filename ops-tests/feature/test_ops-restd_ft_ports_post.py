...
from rest_utils_physical_ft import set_rest_server, add_ip_devices, \
    ensure_connectivity, login_to_physical_switch, SW_IP, HS_IP, USER,\
    PASSWORD, CRT_DIRECTORY_HS, LOGIN_RESULT, HTTP_CREATED, HTTP_OK, \
    HTTP_DELETED, HTTP_BAD_REQUEST
...
def create_port_with_depth_ostl(hs1, step):
    ...
    assert login_result.get('status_code') == LOGIN_RESULT, \
        'ERROR: Status code of login is not {}'.format(LOGIN_RESULT)
