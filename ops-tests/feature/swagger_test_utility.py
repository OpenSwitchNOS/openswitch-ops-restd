...
# ############################################################################
def get_json_file(container_id, ip):
    print("container_id_swagger %s\n" % container_id)
    copy_from_docker = ('docker cp ' + container_id + ':/srv/www/api/'
                        'ops-restapi.json /tmp')
    print(copy_from_docker)

    try:
        res = subprocess.check_output(['scp', '-o',
                'UserKnownHostsFile=/dev/null',
                '-o', 'StrictHostKeyChecking=no',
                'root@' + ip + ':/srv/www/api/ops-restapi.json',
                '/tmp'])
        print(res)
...
# ############################################################################
def get_json_data(container_id, ip):
    get_json_file(container_id, ip)
...
def parse_opsrestapi_json(uri, method_type, json_data):
    ...
    if method_type == "GET":
        ...
        # print("==uri:", uri, "\n", "uri path", uri_path, "==\n")
        print("IN:", ("get" in uri_path))
        assert "get" in uri_path,\
...
# ############################################################################
def swagger_config_model_get(container_id, uri, ip):
...
# ############################################################################
def swagger_config_model_get_id(container_id, uri, ip):
    json_data = get_json_data(container_id, ip)
...
# ##                 the schema datatypes                                #####
# ############################################################################
def swagger_config_model_put(container_id, uri, ip):
    json_data = get_json_data(container_id, ip)
...
# ##                 the schema datatypes                                #####
# ############################################################################
def swagger_config_model_post(container_id, uri, ip):
    json_data = get_json_data(container_id, ip)
...
# ############################################################################
def swagger_model_verification(container_id, uri, method_type, data, ip):
    bugs_flag = False
    swagger_model_schema = model_functions[method_type](container_id, uri, ip)
    ...
    for k, v in swagger_model_schema.items():
        try:
    ...
    # assert bugs_flag == True, "Please look at the issues above and report
    # them
