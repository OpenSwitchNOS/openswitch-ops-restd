...
def delete_port_with_depth(step):
    ...
    assert (
        status_code == http.client.BAD_REQUEST, "Is not sending No Content"
        "status code. Status code: %s" % status_code
    )
...
def delete_port(step):
    ...
    assert (
        status_code == http.client.NO_CONTENT, "Is not sending No Content"
        " status code. Status code: %s" % status_code
    )
...
def test_ops_restd_ft_ports_delete(topology, step, netop_login, sanity_check):
