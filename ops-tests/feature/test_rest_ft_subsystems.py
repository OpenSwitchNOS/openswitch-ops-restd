...
def test_rest_ft_subsystems_ostl(step, setup_module_physical):

    get_result = h1.libs.openswitch_rest.system_subsystems_get(
        https=CRT_DIRECTORY_HS,
        cookies=login_result.get('cookies'),
        request_timeout=5)
    assert get_result.get('status_code') == HTTP_OK
    assert subsys_url in get_result["content"]
