...
def netop_login(request, topology):
    ...
    global vlan_path, vlan, config_selector
...
def test_put_vlan_with_star_etag(step, netop_login, sanity_check):
...
def test_put_vlan_etag_match(step, netop_login, sanity_check):
...
def test_put_vlan_etag_not_match(step, netop_login, sanity_check):
    step(TEST_START % "PUT VLAN with not matching Etag")
    # 1 - Query Resource
    cond_path = vlan + config_selector
    etag, pre_put_get_data = get_etag_and_data(cond_path)
...
def test_post_vlan_etag_match(step, netop_login, sanity_check):
...
def test_post_vlan_etag_not_match(step, netop_login, sanity_check):
...
def test_get_all_vlan_etag_match(step, netop_login, sanity_check):
...
def test_get_all_vlan_etag_not_match(step, netop_login, sanity_check):
...
def test_get_vlan_etag_match(step, netop_login, sanity_check):
...
def test_get_vlan_etag_not_match(step, netop_login, sanity_check):
...
def test_delete_vlan_etag_match(step, netop_login, sanity_check):
...
def test_delete_vlan_etag_not_match(step, netop_login, sanity_check):
