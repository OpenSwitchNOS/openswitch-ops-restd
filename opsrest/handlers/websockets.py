...
from opsrest.websocket.exceptions import WebSocketException, WSInvalidMessage
import json
...
class WebSocketHandler(websocket.WebSocketHandler):
    ...
    def on_message(self, msg_json):
        ...
        error = None
        app_type = None
        status = wsmsg.WS_MSG_STATUS_SUCCESS
        ...
        try:
            ...
            status = wsmsg.WS_MSG_STATUS_ERROR
            error = e.details
            app_log.error(error)

        if app_response_data is not None or error is not None:
            self.send_response(status, app_type, app_response_data, error)
