...
def patch_resource(data, resource, schema, txn, idl, uri):
    ...
    try:
        ...
        (patch, modified) = create_patch(data)
    ...
    try:
        ...
        new_row_json = patch.apply(row_json)
    ...
    app_log.debug("Post-patch pre-hack row_json -> %s" % new_row_json)

    # TODO remove this ugly hack after fix in GET behavior is merged (bug #127)
    new_row_json = remove_empty_optional_columns_hack(schema, resource_update,
                                                      new_row_json)

    app_log.debug("Post-patch post-hack row_json -> %s" % new_row_json)

    try:
        # Verify final transaction data
        verified_data = verify.verify_data(new_row_json, resource, schema,
    ...
    app_log.debug("Verified data -> %s" % verified_data)

    # Update resource with the patched json only if applies
    if resource.relation is not None and modified:
...
def refill_removed_columns(patch, data, resource, schema):
    ...
    for patch_op in patch_list:
        ...
        if patch_op[PATCH_KEY_OP] in (PATCH_OP_REMOVE, PATCH_OP_MOVE):
            ...
            if patch_op[PATCH_KEY_OP] == PATCH_OP_REMOVE:
                ...
                add_path = patch_op[PATCH_KEY_PATH]
            else:
                column = patch_op[PATCH_KEY_FROM].split("/")[1]
                add_path = patch_op[PATCH_KEY_FROM]
            ...
            if column not in data:
                ...
                add_op[PATCH_KEY_PATH] = add_path
    ...
    app_log.debug("PATCH Final patch object %s" % patch_fix.to_string())
