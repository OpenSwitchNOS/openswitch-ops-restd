...
def _get_base_uri():
    ...
    return constants.OVSDB_BASE_URI
...
def filter_get_results(get_data, filters):
    filtered_data = []
...
def sort_get_results(schema, table, get_data, sort_by_columns, reverse_=False):
    ...
    sorted_data = sorted(
        get_data,
        key=lambda item: tuple(item[k] for k in sort_by_columns),
        reverse=reverse_)
...
def categorize_get_data(schema, table, data):
    ...
    stats_keys = schema.ovs_tables[table].stats
    status_keys = schema.ovs_tables[table].status
    references_keys = schema.ovs_tables[table].references
    # TODO Change this when Reference has the Category
    # attribute and add a function that recategorizes data
    config_keys.update(references_keys)
