...
import json
from .exceptions import WSInvalidMessage
...
def _validate_mandatory_fields(message):
    ...
    elif WS_APPLICATION not in message:
        missing_field = WS_APPLICATION
...
def validate(message):
    ...
    if not isinstance(message, dict):
        raise WSInvalidMessage("Message not in JSON format.")
...
def _init_message(type, app_type, data):
    ...
    message[WS_APPLICATION] = app_type

    if data:
        message[WS_APPLICATION_DATA] = data
