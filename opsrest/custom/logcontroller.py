...
class LogController(BaseController):
        ...
        if not(since_until_arg[0] in time_relative_keywords):
            if len(since_until_arg) == 2:
                if not((since_until_arg[1] in time_keywords and
                       ...
                       (re.search(r'\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d',
                        arg) is not None)):
    ...
    def validate_priority(priority, error_messages):
        if int(priority) > 7 or int(priority) < 0:
                error_messages.append("Invalid log level. Priority should be" +
                                      "positive and less than or equal to 7:" +
                                      "% s" % priority)
    ...
    def validate_args_data(self, query_args):
        ...
        if offset is not None:
            if not(offset.isdigit() and offset > 0):
        ...
        if limit is not None:
            if not(limit.isdigit() and limit > 0):
                error_messages.append("Only positvie integers are allowed" +
                                      "for limit")
    ...
    def handle_after_cursor(query_args):
        i = ';i='+ str(getutils.get_query_arg('i', query_args))
        b = ';b=' + str(getutils.get_query_arg('b', query_args))
        m = ';m=' + str(getutils.get_query_arg('m', query_args))
        t = ';t=' + str(getutils.get_query_arg('t', query_args))
        x = ';x=' + str(getutils.get_query_arg('x', query_args))
        del query_args['i']
        del query_args['b']
        del query_args['m']
        del query_args['t']
        del query_args['x']
        query_args[REST_LOGS_PARAM_AFTER_CURSOR][0] += (i + b + m + t + x)
    ...
    def get_all(self, current_user, selector=None, query_args=None):
        ...
        if response:
            ...
            if REST_QUERY_PARAM_OFFSET in query_args:
                offset = int(getutils.get_query_arg(REST_QUERY_PARAM_OFFSET, query_args))
            if REST_QUERY_PARAM_LIMIT in query_args:
                limit = int(getutils.get_query_arg(REST_QUERY_PARAM_LIMIT, query_args))
                if limit > MAXLIMIT:
                    limit = MAXLIMIT
