...
def get_param_list(query_arguments, query_param_type):
    arguments = []
    if query_arguments is not None and \
            query_param_type in query_arguments:
        arguments = query_arguments[query_param_type]
...
def post_process_get_data(get_data, sorting_args, filter_args, offset, limit,
    ...
    if columns_args:
        processed_get_data = columns_retrieval_get_results(processed_get_data,
                                                           columns_args)
...
def columns_retrieval_get_results(get_data, retrieve_by_columns):
    for row in get_data:
