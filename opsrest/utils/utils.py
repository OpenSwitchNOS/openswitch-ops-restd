...
def get_parent_trace(table_name, row, schema, idl):
    ...
    while table.parent is not None and row is not None:
        ...
        if relation == OVSDB_SCHEMA_BACK_REFERENCE:
            ...
            row, _ = get_parent_row(parent_table.name, row, column,
                                    schema, idl)
...
def get_columns_to_values(columns, idl, schema, row, table, resource_uri):
    column_to_values = {}
    ...
    for column in columns:
        ...
        if column in schema_table.references:
            data = get_column_json(column, row, table, schema, idl,
                                   resource_uri)
        else:
            ovs_column = None
            ...
            resource_row = idl.tables[table].rows[row]
