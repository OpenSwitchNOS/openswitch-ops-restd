...
def is_subtree_configurable(table_name, restschema, idl):
    """
    ...
    if len(restschema.ovs_tables[table_name].config.keys()) > 0:
        return True
    # references
    _references = restschema.ovs_tables[table_name].references
    for column_name, column in _references.iteritems():
        if column.relation == OVSDB_SCHEMA_REFERENCE:
            return True
    # children
    for column_name in restschema.ovs_tables[table_name].children:
        if column_name not in _references:
            child_table = column_name
        else:
            child_table = restschema.ovs_tables[table_name].\
                references[column_name].ref_table
        if is_subtree_configurable(child_table):
            return True
