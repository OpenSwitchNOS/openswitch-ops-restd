...
def row_to_kv_index(parent_row, row, column, idl):
    column_item = parent_row.__getattr__(column)
    for key, value in column_item.iteritems():
        if row.uuid == value.uuid:
            return key
...
def get_table_key(row, table_name, schema, idl):
    ...
    if table.parent:
        parent_table = schema.ovs_tables[table.parent]
        column_ref = get_parent_column_ref(parent_table.name, table.name, schema)
        if parent_table.references[column_ref].kv_type:
            parent_row = get_parent_row(parent_table.name, row, column_ref, schema, idl)
            key = row_to_kv_index(parent_row, row, column_ref, idl)
