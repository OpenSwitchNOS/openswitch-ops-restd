...
class NotificationHandler():
    ...
    def create_subscription(self, subscription_name, subscription_row,
        ...
        subscriber_row, _ = utils.get_parent_row(consts.SUBSCRIBER_TABLE,
                                                 subscription_row,
                                                 subscriber_ref_col,
                                                 self._schema, idl)
        ...
        for table, subscriptions in self._subscriptions_by_table.iteritems():
            ...
            for subscription in subscriptions:
                try:
                    ...
                    self._add_updates(changes, consts.UPDATE_TYPE_ADDED, added)
                    self._add_updates(changes, consts.UPDATE_TYPE_MODIFIED,
                                      modified)
                    self._add_updates(changes, consts.UPDATE_TYPE_DELETED,
                                      deleted)

                    if changes:
                        subs_name = subscription.subscriber_name
                        subscriber_notifications[subs_name] = changes
