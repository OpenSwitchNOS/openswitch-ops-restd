...
class OvsdbConnectionManager:
    ...
    def idl_init(self):
        while not self.idl.has_ever_connected():
            ...
            self.idl.run()
        self.ovssock = self.idl._session.rpc.stream.socket
        self.add_fd_callbacks(self.ovssock.fileno())
    ...
    def idl_reconnect(self):
        ...
        try:
            ...
            # idl will raise an error exception if cannont reconnect
            app_log.debug("Adding callback to idl_reconnect")
            IOLoop.current().add_timeout(time.time() + self.timeout,
                                         self.idl_reconnect)

    def add_fd_callbacks(self, fd):
        IOLoop.current().add_handler(fd, self.idl_run,
    ...
    def check_transactions(self):
        ...
        for tx in self.transactions.txn_list:
            tx.commit()
