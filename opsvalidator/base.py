...
class ValidationArgs(object):
    ...
    def __init__(self, idl, schema, resource, data):
        ...
        if resource.next is not None:
            ...
            self.p_resource_idl = idl.tables[self.p_resource.table]
            self.p_resource_row = self.p_resource_idl.rows[self.p_resource.row]
        ...
        self.resource_idl = idl.tables[self.resource.table]

        # New resources will not have an associated row
        if self.resource.row is not None:
            self.resource_row = self.resource_idl.rows[self.resource.row]
        else:
