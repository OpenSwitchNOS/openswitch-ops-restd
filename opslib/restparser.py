...
class OVSReference(OVSColumn):
    ...
    def __init__(self, table_name, column_name, ovs_base_type,
        ...
        if relation not in ["child", "parent", "reference"]:
    ...
    def from_json(json, name):
        ...
        for column_name, column_json in columns_json.iteritems():
            ...
            if relationship == "1:m":
                ...
                table.config[column_name] = OVSColumn(table.name, column_name,
                                                      type_, is_optional,
                                                      mutable, category)
            elif category == "status":
                table.status[column_name] = OVSColumn(table.name, column_name,
                                                      type_, is_optional,
                                                      True, category)
            elif category == "statistics":
                table.stats[column_name] = OVSColumn(table.name, column_name,
                                                     type_, is_optional,
                                                     True, category)
