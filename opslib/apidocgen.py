...
def genGetParams(table, is_instance=False):
    ...
    param["in"] = "header"
    param["description"] = "entity-tag value for representation comparison (see RFC 7232 - Conditional Requests - section 3.2)"
...
def genGetResource(table, parent_plurality, parents, resource_name, is_plural):
    ...
    response = {}
    response["description"] = "OK"
    response["headers"] = {'ETag': {'description': 'The current entity-tag for the selected representation (see RFC 7232 - Conditional Requests - section 2.3)', 'type': 'string'} }
    schema = {}
    schema["type"] = "array"
    item = {}
...
def genPutInstance(table, parent_plurality, parents, resource_name, is_plural):
    if table.config:
        ...
        param["name"] = "If-Match"
        param["in"] = "header"
        param["description"] = "entity-tag value for representation comparison (see RFC 7232 - Conditional Requests - section 3.1)"
        param["required"] = False
        param["type"] = "string"
        params.append(param)
