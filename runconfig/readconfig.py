...
def _get_row_data(row, table_name, schema, idl, index=None):
    ...
    for child_name in table_schema.children:
        ...
        if child_name in table_schema.references:
            ...
            if table_schema.references[child_name].category != OVSDB_SCHEMA_CONFIG:
                continue
            ...
            for item in column_data:
                if kv_type:
                    kv_index = keys[count]
                    data = _get_row_data(item, child_table_name, schema, idl, kv_index)
        ...
        # Backward reference
            ...
            for name, column in schema.ovs_tables[child_name].references.iteritems():
            ...
            for item in idl.tables[child_name].rows.itervalues():
                ...
                # Parent reference is same as 'row' (row was passed to this function)
                # this is now the child of 'row'
    ...
    for refname, refobj in table_schema.references.iteritems():
        ...
        refdata = []
        if refobj.relation == OVSDB_SCHEMA_REFERENCE and refobj.category == OVSDB_SCHEMA_CONFIG :
            ...
            for item in reflist:
                key_index = utils.row_to_index(item, ref_table_name, schema, idl)
