...
class LogsSinceUntilTest (OpsVsiTest):
    ...
    def logs_with_since_relative_filter(self):
        ...
        status_code, response_data = execute_request(self.LOGS_PATH, "GET", None,
                                                     self.SWITCH_IP)
        ...
        assert len(json_data) <= LIMIT_TEST, "Pagination limit failed %s " % len(json_data)
        info("### Pagination for logs works fine ###\n")

        assert self.verify_timestamp(json_data), "Logs for incorrect timestamp also displayed"
        info("### Since filter for logs is working fine ###\n")

        info("\n########## End Test to Validate logs with since relative filter" +
             "##########\n")
    ...
    def logs_with_until_relative_filter(self):
        ...
        info("\n########## End Test to Validate logs with until relative filter" +
             "##########\n")
    ...
    def logs_with_until_timestamp_filter(self):
        ...
        info("until test ---> %s" % until_test)

        self.LOGS_PATH = self.PATH + "?until=%s&offset=%s&limit=%s" % \
                         (until_test, OFFSET_TEST, LIMIT_TEST)
        info("log path ---> %s" % self.LOGS_PATH)
        status_code, response_data = execute_request(self.LOGS_PATH, "GET", None,
                                                     self.SWITCH_IP)
