...
class systemTest(HalonTest):
    ...
    def get_switch_ip(self):
        ...
        return s1.cmd("python -c \"import socket; print socket.gethostbyname(socket.gethostname())\"")

    def execute_request(self, switch_ip, http_method, request_data=json.dumps("")):
    ...
    def call_system_get(self, switch_ip):
        info("\n########## Executing GET request on %s ##########\n" % path)

        # # Execute GET

        response, json_string = self.execute_request(switch_ip, "GET")
    ...
    def call_system_options(self, switch_ip):
        info("\n########## Executing OPTIONS request on %s ##########\n" % path)

        # # Execute OPTIONS

        response, json_string = self.execute_request(switch_ip, "OPTIONS")
    ...
    def call_system_put(self, switch_ip):
        info("\n########## Executing PUT request on %s ##########\n" % path)

        # # Get initial data

        response, pre_put_json_string = self.execute_request(switch_ip, "GET")
        ...
        response, json_string = self.execute_request(switch_ip, "PUT", json.dumps({'configuration': put_data}))

        assert response.status == httplib.OK

        # # Get post-PUT data

        response, post_put_json_string = self.execute_request(switch_ip, "GET")
        ...
        response, json_string = self.execute_request(switch_ip, "PUT", json_string)
...
class Test_system:
    ...
    def test_run (self):
        switch_ip = self.test_var.get_switch_ip()
        self.test_var.call_system_get(switch_ip)
        self.test_var.call_system_options(switch_ip)
        self.test_var.call_system_put(switch_ip)
