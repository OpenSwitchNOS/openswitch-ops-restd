...
class OpsIdl(Idl):
    ...
    def _Idl__process_update(self, table, uuid, old, new):
        ...
        if not new:
            # Delete row.
            if row:
                self._delete_from_index_map(row, table)
                del table.rows[uuid]
                changed = True
        ...
        if _is_new_row:
            self._update_index_map(uuid, table, new)

        return changed

    def _update_index_map(self, uuid, table, new):
    ...
    def _delete_from_index_map(self, row, table):
        index = self.row_to_index_lookup(row, table)
        if index in table.index_map:
