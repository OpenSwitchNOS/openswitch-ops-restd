...
def setup_validators(extschema, idl):
    if not validator:
        validator = ValidatorAdapter(extschema, idl)
...
def setup_row(rowdata, table_name, extschema, idl, txn, row=None, parent=None, parent_table=None):
    ...
    if row is None:
        ...
        ops.utils.set_config_columns(row_data, row, table_name, extschema, idl)
    ...
    op = ops.constants.REQUEST_TYPE_CREATE if new else ops.constants.REQUEST_TYPE_UPDATE
    validator.add_resource_op(op, row, table_name)
