...
def setup_table(table_name, data, extschema, idl, txn):
    ...
    # if table is missing in config json
    if table_name not in data and mutable:
            ...
            if not _new:
                ...
                if new_data is not None:
                    for x,y in new_data.iteritems():
                        ...
                        split_x.insert(extschema.ovs_tables[key].index_columns.index(column_name),row.uuid)
                        tmp = []
                        for _x in split_x:
