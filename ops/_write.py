        ...
        if key in table_schema.references:
            ...
            if key not in row_data or not row_data[key]:
                ...
                if _min == 1 and _max == 1 and not kv_type:
                    ...
                    if not kv_type:
                        (_child, is_new, unchanged) = setup_row(new_data,
                                                                child_table_name,
                                                                extschema, idl,
                                                                txn,
                                                                validator_adapter)
                ...
                # kv type children references
                    ...
                    for index, child_data in new_data.iteritems():
                        ...
                        if index in column_data:
                            (_child, is_new, unchanged) = setup_row(child,
                                                                    child_table_name,
                                                                    extschema, idl,
                ...
                # list type children references
                    ...
                    children = {}
                    for index, child_data in new_data.iteritems():
                        (_child, is_new, unchanged) = setup_row({index:child_data},
                                                                child_table_name,
                                                                extschema, idl,
                                                                txn,
                                                                validator_adapter)
            ...
            if not _new:
                ...
                if new_data is not None:
                    for x,y in new_data.iteritems():
                        ...
                        x = '/'.join(tmp)
                        (child, is_new, unchanged) = setup_row({x:y}, key,
                                                               extschema, idl,
                                                               txn,
                                                               validator_adapter)
