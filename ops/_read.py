        ...
        if (
            ...
            if isinstance(references, ovs.db.idl.Row):
                if not (_min==1 and _max==1):
                    raise Exception('datatype for column %s in table %s must be a UUID' %(refname, table_name))
                else:
                    refdata = ops.utils.row_to_index(
                            references, ref_table_name, schema, idl)
            elif isinstance(references, dict):
                refdata = {}
                for key,ref in references.iteritems():
                    ref_index = ops.utils.row_to_index(
                            ref, ref_table_name, schema, idl)
                ...
                raise Exception('datatype for column %s in table %s is not supported' %(refname, table_name))
